...
import { approveBooking, getAdminBookingDetails, rejectBooking } from '@/services/adminService';
...
export function BookingReviewScreen({ route, navigation }: Props) {
  ...
  const onApprove = async () => {
    ...
    try {
      ...
      Alert.alert('Approved', 'Booking approved and user notification created.');
  ...
  const onReject = async () => {
    if (!booking) return;
    if (!remarks.trim()) {
      setError('Admin remarks are required to reject a booking.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Rejected', 'Booking rejected and user notification created.');
      navigation.navigate('PendingRequests');
    } catch (rejectError) {
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {canReview ? (
        ...
        <Text style={styles.closedText}>This request has already been reviewed.</Text>
      )}
    </ScrollView>
  );
}
