...
async function processJob(supabase: ReturnType<typeof createClient>, job: ReceiptEmailJobRow, workerId: string) {
  ...
  try {
    ...
    if (failed && receiptForError) {
      const booking = await fetchBooking(supabase, receiptForError.booking_id).catch(() => null);
      if (booking) {
        await notifyReceiptEmailFailed({ supabase, receipt: receiptForError, booking }).catch((notifyError) => {
          console.warn('[receipt-email] failure notification unavailable:', sanitizeError(notifyError instanceof Error ? notifyError.message : String(notifyError)));
        });
      }
    }
...
async function notifyReceiptEmailFailed(params: {
  supabase: ReturnType<typeof createClient>;
  receipt: ReceiptRow;
  booking: BookingRow;
}) {
  ...
  if (error) {
    console.warn('Unable to insert receipt email failure notification:', sanitizeError(error.message));
  }
}
